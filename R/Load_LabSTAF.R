...
#' @export
load.LabSTAF_DJB <- function(directory) {
  result <- list()
  ...
  for (file_path in files) {
    ...
    # finalize
    result[[file_name]] <- list(Params = Params_t, Notes = Notes_t, rPE = rPE_t, A = Up, B = Dark, C = Down, Datetime = datetime, File = file_name)
...
# Title: LabSTAF Loading function for my files lmafo
#' Load LabSTAF
